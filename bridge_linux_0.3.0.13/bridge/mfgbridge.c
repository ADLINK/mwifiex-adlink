...
#define BRIDGE_VERSION "0.3.0.13"
...
static unsigned char uart_buf[3072] = { 0 };    // uart buffer
...
char RAW_UART_PORT[128] = {0};      /* default hci Uart port */

int DUT_15_4_If_Type = 0;                      /* default iterfavce is Uart port*/
int DUT_15_4_GPIO_Pin = 12;
char DUT_15_4_GPIO_PORT[128] = "/dev/gpiochip0";
char DUT_15_4_PORT[128] = "/dev/ttyUSB0";      /* default hci Uart port */
...
extern int zigbee_rx_per_enable; 
/********************************************************
        Local Functions
********************************************************/
static void
...
int brdg_parse_config(bridge_cb * bridge, char *input_conf_file, drv_config * drv_conf)
{
    char input_line[200];
    FILE *input;
    char *p, temp[5];
    ...
    while (fgets(input_line, 100, input)) {
        ...
        if (strstr(input_line, "WLAN_interface_count")) {
         ...
         if (strstr(input_line, multi_wlan_ifname)) {
            ...
            n = get_string(++p, (char *)(&MultiDevPtr->wlan_ifname));
            if (n == -1)
                goto invalid_format_error;
        ...
        if (strstr(input_line, "Bt_If_Mode")) {
            char bt_if_mode_temp[4];
            memset(bt_if_mode_temp, 0, 4);
            drv_conf->BT_IF_Mode = 0;
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
            n = get_string(++p, bt_if_mode_temp);
            if (n == -1)
                goto invalid_format_error;
            if(n > 1)
                goto invalid_format_error;
            drv_conf->BT_IF_Mode = bt_if_mode_temp[0] - '0';
            switch(drv_conf->BT_IF_Mode)
            {
                case 0: printf("BT in HCI Mode..........\n");
                    break;
                case 1: printf("BT in MBTChar Mode..........\n");
                    break;
                case 2: printf("BT in RAWUART Mode..........\n");
                    break;
                default:
                printf("Invalid BT_IF_MODE : %d",drv_conf->BT_IF_Mode);
                return -1;
            }
            continue;
        }
...
//15_4
    ...
    if (strstr(input_line, "DUT_15_4_If_Type")) {
            DUT_15_4_If_Type = 0;
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
            DUT_15_4_If_Type = atoi(++p);
            printf("~~DUT_15_4_If_Type = %d\n", DUT_15_4_If_Type);
            continue;
        }

    if (strstr(input_line, "DUT_15_4_GPIO_Pin")) {
            DUT_15_4_GPIO_Pin = 0;
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
            printf("##NFO - GPIO pin info\n");
            DUT_15_4_GPIO_Pin = atoi(++p);
            printf("~~DUT_15_4_If_Type = %d\n", DUT_15_4_GPIO_Pin);
            continue;
        }

    if (strstr(input_line, "DUT_15_4_GPIO_Port")) {
            memset(DUT_15_4_GPIO_PORT, 0, 128);
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
            n = get_string(++p, DUT_15_4_GPIO_PORT);
            printf("##NFO - GPIO port info\n");
            printf("~~%s\n", DUT_15_4_GPIO_PORT);
            if (n == -1)
                goto invalid_format_error;
            continue;
	...
	if (strstr(input_line, "DUT_15_4_port")) {
            memset(DUT_15_4_PORT, 0, 128);
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
            n = get_string(++p, DUT_15_4_PORT);
			printf("~~%s\n", DUT_15_4_PORT);
            if (n == -1)
                goto invalid_format_error;
            continue;
...
//            cmd_payload_len = *msglen - sizeof(cmd_header);
        ...
        //;printf("HCIDeviceID=0x%x\n",HCIDeviceID);
        ret =
            bridge->drv_proc_zigbee_cmd(bridge->drv, cmd_payload,
                                     &cmd_payload_len, buflen, HCIDeviceID);

        /* Update command header information */
        cmd_hd->status = ret;
        cmd_hd->length = cmd_payload_len + sizeof(cmd_header);
	//;	printf("ZIGBEE>>=0x%x, 0x%x\n", cmd_hd->length, cmd_payload_len);
        *msglen = cmd_payload_len + sizeof(cmd_header);
        break;
    default:
...
#endif
    bridge->drv_proc_zigbee_cmd = drv_proc_ziebee_command;
...
//    unsigned char            buf[MAXBUF];
    ...
    if (brdg_parse_config(&Bridge, "bridge_init.conf", &Drv_Config) != 0) {
        /* Failed to parse */
        return 0;
    }
    //Debug

...
#define BRIDGE_VERSION "0.1.0.43"
...
static char UART_PORT[256] = "/dev/ttyS0";      /* Default Uart port */
...
int
get_string(const char *pos, char *buf)
{
...
int
brdg_parse_config(bridge_cb * bridge, char *input_conf_file)
{
    char input_line[200];
    FILE *input;
    char *p, temp[5];
    ...
    while (fgets(input_line, 100, input)) {
        ...
        if (strstr(input_line, "WLAN_interface_name")) {
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
        ...
        if (strstr(input_line, "BT_interface_name")) {
            p = strchr(input_line, '=');
            if (!p)
                goto invalid_format_error;
            n = get_string(++p, drv_conf->hci_port);
            if (n == -1)
                goto invalid_format_error;
            continue;
        }

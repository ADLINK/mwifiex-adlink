...
get_private_info(const char *ifName)
{
	...
	if (ioctl(sockfd, SIOCGIWPRIV, &iwReq) < 0) {
...
#ifdef NONPLUG_SUPPORT
    sprintf(drv_conf->hci_port, "hci0");
#endif
...
drv_init(struct _bridge_cb *bridge, drv_config * drv_conf)
{
...
#ifdef NONPLUG_SUPPORT
    drv->hci_ifname = drv_conf->hci_port;
#endif
    ...
    if (!drv_wrapper_get_info(drv->wlan_ifname))
        drv_wrapper_init(drv->wlan_ifname);
#ifdef NONPLUG_SUPPORT
    /** Init BT */
    if (!drv_wrapper_get_hci_info(drv->hci_ifname))
        drv_wrapper_init_hci(drv->hci_ifname);
#endif
...
drv_proc_wlan_command(drv_cb * drv, unsigned char *buf, int *rsplen, int buflen)
{
    ...
    if (ethio_flag == 1) {
        ...
        *rsplen = 0;
        ...
        mfg_dprint(DBG_GINFO, "DRV:  send host cmd thr ioctl\n");
